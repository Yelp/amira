...
class TestOSXCollectorDataProcessor:
    ...
    def test_process_input_malformed_gz(self, tar_gz_mock):
        ...
        logging.error.assert_has_calls(
            [call("Failed to read the archive: mock.tar.gz is not a gz file")],
        )
    ...
    def test_perform_analysis(self):
        with patch("amira.data_processor.AnalyzeFilter") as mock_filter, patch(
            "amira.data_processor.output_filter",
        ) as mock_run_filter:
            processor = OSXCollectorDataProcessor()
